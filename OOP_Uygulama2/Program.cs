...
Console.WriteLine("Tüm Yazarları Listele:");
...
// Tüm kategorileri listeleyen kod
...
// Belirli bir yazarın kitaplarını listeleyen kod
Console.WriteLine("\nYazar ID'si 2 olan yazarın kitaplarını listele:");
var booksByAuthor = bookRepository.GetAll().Where(book => book.AuthorId == 2).ToList();
booksByAuthor.ForEach(book => Console.WriteLine($"Title: {book.Title}, Author ID: {book.AuthorId}"));

// Belirli bir kategorideki kitapları listeleyen kod
Console.WriteLine("\nKategori ID'si 1 olan kategorideki kitapları listele:");
var booksByCategory = bookRepository.GetAll().Where(book => book.CategoryId == 1).ToList();
booksByCategory.ForEach(book => Console.WriteLine($"Title: {book.Title}, Category ID: {book.CategoryId}"));
